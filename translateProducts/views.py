...
def adminProductView(request):
    ...
    if request.method == "DELETE":
        equipo =request.data['equipo']
        raise AuthenticationFailed('errorcito')
    ...
    if request.method == "PUT":
        ...
        iva = request.data['iva']
        active = request.data['active']
        data = db.read_item('traduccion_equipos_prepago',equipo)
        data['stok'] = stok
        data['iva']=iva
        data['active']=active
            ...
            try:
                ...
                if traductor[nombre]['active'] == '1' and nombre2 not in lista:
                    if validate:
                        ...
                        raise AuthenticationFailed(itemData[0][0])
                        crediminuto.append(str(itemData[1])[:-2])
            except KeyError:
